...
Dialog {
    ...
    function openForTopic(id, title, dateIso, notes) {
        topicId = id;
        titleField.text = title || "";
        initialDateIso = (dateIso && dateIso.length > 0) ? dateIso : backend.todayDateIso;
        dateField.text = initialDateIso;
    ...
    function submit() {
        if (titleField.text.trim().length > 0 && root.topicId > 0) {
            backend.updateTopic(root.topicId, titleField.text.trim(), notesField.text, root.initialDateIso);
            root.accept();
        }
    }
